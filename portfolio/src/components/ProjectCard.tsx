...
export default function ProjectCard({
  ...
  const isCardLink = Boolean(link && !placeholder);

  const shellClass = `group relative flex flex-col rounded-2xl border transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
    placeholder
    ...
    isCardLink
      ? "cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-slate-900"
      : ""
  }`;
  ...
  const body = (
    <div className="p-6 flex-1 flex flex-col">
      ...
      {link && (
        <div className="mt-6">
          {isCardLink ? (
            <span className="inline-flex items-center gap-2 text-sm font-semibold text-primary-600 dark:text-primary-400 group-hover:text-primary-700 dark:group-hover:text-primary-300 transition-colors">
              {linkLabel}
  ...
  if (isCardLink && link) {
    return (
      <a
        ...
        aria-label={`${title} — ${linkLabel} (opens in new tab)`}
      >
        {body}
      </a>
    );
